...
draw_dashboard() {
    ...
    echo -e "$B_MID_T_DN"
    row2 "${G}[1]${NC} LIMITS MANAGEMENT"   "${G}[6]${NC} HARDWARE OPTIMIZE"
...
speed_limit_manager() {
    while true; do
        clear; hline; center "${C}💠 LIMIT MANAGEMENT${NC}"; hline; echo
...
anti_share_manager() {
    while true; do
        ...
        echo -e "  ${G}[1]${NC} LIMIT TCP CONNECTIONS"
...
# ── 0. Script Manager (FIXED UPDATE WITH VERSION COMPARE) ─────────────────────
script_manager() {
    while true; do
        ...
        case "$sm_choice" in
            1) 
                ...
                # Extract numeric part for comparison (e.g., "0.0.8" from "0.0.8 BETA")
                local local_num=$(echo "$VERSION" | grep -oE '^[0-9]+\.[0-9]+\.[0-9]+')
                local remote_num=$(echo "$REMOTE_VERSION" | grep -oE '^[0-9]+\.[0-9]+\.[0-9]+')
                if [[ -z "$local_num" ]]; then local_num="$VERSION"; fi
                if [[ -z "$remote_num" ]]; then remote_num="$REMOTE_VERSION"; fi

                echo -e "\n  ${DG}------------------------------------------------------------${NC}"
                echo -e "  ${C}▶ CURRENT VERSION : ${W}${VERSION}${NC}"

                # Compare versions
                version_compare "$local_num" "$remote_num"
                case $? in
                    0)
                        ...
                        ;;
                    1)
                        echo -e "  ${C}▶ LATEST VERSION  : ${Y}${REMOTE_VERSION} ${R}(DOWNGRADED)${NC}"
                        echo -e "  ${DG}------------------------------------------------------------${NC}\n"
                        echo -e "  ${Y}⚠  YOUR LOCAL VERSION IS NEWER THAN THE REMOTE.${NC}"
                        echo -e "  ${Y}   DO YOU WANT TO DOWNGRADE TO THE OLDER VERSION?${NC}\n"
                        confirm "INSTALL OLDER VERSION? (DATA WILL BE SAFE)" || continue
                        ;;
                    2)
