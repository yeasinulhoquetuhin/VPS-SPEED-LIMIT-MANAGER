...
script_manager() {
    while true; do
        ...
        case "$sm_choice" in
1) 
    # Define ANSI color codes exactly as in the screenshot
    C='\033[1;36m'   # Cyan (headers, borders, arrows)
    NC='\033[0m'     # No color
    DG='\033[1;30m'  # Dark gray (separators, subtle text)
    R='\033[1;31m'   # Red (errors)
    Y='\033[1;33m'   # Yellow (version numbers, warnings)
    G='\033[1;32m'   # Green (success, checkmarks)
    M='\033[1;35m'   # Magenta (update available notice)
    ...
    # ✅ FIX: Download the MAIN script, not the installer
    local REPO_URL="https://raw.githubusercontent.com/yeasinulhoquetuhin/VPS-SPEED-LIMIT-MANAGER/refs/heads/master/clp.sh"
    local REMOTE_RAW=$(curl -s --max-time 5 "$REPO_URL")
    
    local REMOTE_VERSION=$(echo "$REMOTE_RAW" | grep -m 1 '^VERSION=' | tr -d '\r' | cut -d'"' -f2)
    ...
    echo -e "\n  ${DG}------------------------------------------------------------${NC}"
    echo -e "  ${C}▶ CURRENT VERSION : ${Y}${VERSION}${NC}"
    
    if [[ "$VERSION" == "$REMOTE_VERSION" ]]; then
        echo -e "  ${C}▶ LATEST VERSION  : ${G}${REMOTE_VERSION} (UP TO DATE)${NC}"
        echo -e "  ${DG}------------------------------------------------------------${NC}\n"
    ...
    if [[ -s "$TEMP_SCRIPT" ]]; then
        (
            if ! grep -q "TDZ NETWORK CONTROL" "$TEMP_SCRIPT"; then
                base64 -d "$TEMP_SCRIPT" > "${TEMP_SCRIPT}.dec" 2>/dev/null
                mv "${TEMP_SCRIPT}.dec" "$TEMP_SCRIPT"
            fi
