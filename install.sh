...
run_installer() {
    ...
    (touch "$INSTALL_FLAG") & spin $! "SAVING INSTALLATION STATE"
    ...
    echo; hline "─" "$DG"; echo; center "${G}✔  INSTALLATION COMPLETED SUCCESSFULLY!${NC}"; echo
    ...
    while true; do
        ...
        case "$sm_choice" in
            1) 
                ...
                echo -e "  ${O}└──────────────────────────────────────────────────────────┘${NC}"
                echo
                confirm "UPDATE SCRIPT? (DATABASE WILL BE SAFE)" || return
                echo
                ...
                echo
                echo -e "  ${G}✔ SCRIPT UPDATED SUCCESSFULLY!${NC}"
                echo
                center "${DG}PLEASE RESTART THE DASHBOARD BY TYPING: ${C}clp${NC}"
                echo
                hline
                echo -e "  ${DG}PRESS ENTER TO EXIT...${NC}"
                read -r
                exit 0
                ;;
            2)
                echo; confirm "REINSTALL LOCALLY? (WILL CLEAR CURRENT RULES)" || return; echo
                ...
                echo "#!/bin/bash" > "$RULES_FILE"; rm -f "$INSTALL_FLAG"; > "$DB_FILE"
                echo -e "\n  ${G}✔ READY FOR REINSTALL. PRESS ENTER...${NC}"; read -r; run_installer
                ;;
            3) uninstall_clp ;;
            0) return ;;
            *) sleep 1 ;;
