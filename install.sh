    ...
    while true; do
        ...
        case "$sm_choice" in
            1) 
                ...
                # Smart Check: Base64 & Plain Text
                local REMOTE_VERSION=$(echo "$REMOTE_RAW" | base64 -d 2>/dev/null | grep -m 1 '^VERSION=' | cut -d'"' -f2)
                if [[ -z "$REMOTE_VERSION" ]]; then
                    REMOTE_VERSION=$(echo "$REMOTE_RAW" | grep -m 1 '^VERSION=' | cut -d'"' -f2)
                fi
                
                # STRICT GUARDRAIL: Prevent Fork Bomb if wrong file is detected!
                if [[ -z "$REMOTE_VERSION" ]]; then
                    echo -e "  ${DG}────────────────────────────────────────────────────────────${NC}"
                    echo -e "\n  ${R}✘ CRITICAL ERROR: COULD NOT DETECT VERSION IN GITHUB SCRIPT.${NC}"
                    echo -e "  ${Y}⚠ REASON: THE URL POINTS TO A WRAPPER OR INCORRECT FILE.${NC}"
                ...
                if [[ -s "$TEMP_SCRIPT.raw" ]]; then
                    (
                        if grep -q "TDZ NETWORK CONTROL" "$TEMP_SCRIPT.raw"; then
                            mv "$TEMP_SCRIPT.raw" "$TEMP_SCRIPT"
                        else
                            base64 -d "$TEMP_SCRIPT.raw" > "$TEMP_SCRIPT" 2>/dev/null
