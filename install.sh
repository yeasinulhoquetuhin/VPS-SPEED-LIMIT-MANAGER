...
run_installer() {
    ...
    # --- CHECKING SECTION WITH THIN LINES ---
    echo -e "  ${DG}────────────────────────────────────────────────────────────${NC}"
    (
    ...
    # Show which version is being installed
    ...
    # Proceed with the actual installation steps
    (
    ...
    echo; hline "─" "$DG"; echo; center "${G}✔  SYSTEM READY! CLP INSTALLED SUCCESSFULLY.${NC}"; echo
    ...
    while true; do
        ...
        case "$sm_choice" in
            1) 
                ...
                # Fetching raw data from GitHub safely
                local REPO_URL="https://raw.githubusercontent.com/yeasinulhoquetuhin/VPS-SPEED-LIMIT-MANAGER/refs/heads/master/install.sh"
                local REMOTE_RAW=$(curl -s --max-time 5 "$REPO_URL")
                
                # Smart Version Check: Supports BOTH Base64 and Plain Text automatically!
                ...
                if [[ -s "$TEMP_SCRIPT.raw" ]]; then
                    (
                        # Smart Decode: Check if Base64 or Plain Text
                        ...
                        # Optimize script format for Linux
