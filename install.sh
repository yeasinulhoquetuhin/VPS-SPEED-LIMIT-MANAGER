...
run_installer() {
    ...
    (
        local REPO_URL="https://raw.githubusercontent.com/yeasinulhoquetuhin/VPS-SPEED-LIMIT-MANAGER/refs/heads/master/install.sh"
...
# ── TCP LIMIT ON PORT (with port 22 protection) ──
anti_share_manager() {
    while true; do
...
# ── FIXED: Proper Bandwidth Unlimit with Animations ──
...
# ── 2. Firewall Manager (with port 22 protection) ────────────────────────────
firewall_manager() {
    while true; do
    ...
    while true; do
        ...
        case "$sm_choice" in
             1) 
                ...
                local REPO_URL="https://raw.githubusercontent.com/yeasinulhoquetuhin/VPS-SPEED-LIMIT-MANAGER/refs/heads/master/install.sh"
                local TEMP_RAW="/tmp/tdz_raw_$$.txt"
                local TEMP_DEC="/tmp/tdz_dec_$$.sh"
                
                # Download raw file directly to a temporary file
                curl -s --max-time 5 "$REPO_URL" > "$TEMP_RAW"
                
                # Bulletproof Smart Decode Logic
                if base64 -d "$TEMP_RAW" > "$TEMP_DEC" 2>/dev/null; then
                    if ! grep -q "TDZ NETWORK CONTROL" "$TEMP_DEC"; then
                        cp "$TEMP_RAW" "$TEMP_DEC" # Fallback to plain text
                    fi
                else
                    cp "$TEMP_RAW" "$TEMP_DEC" # Fallback to plain text
                fi
                
                # Extract version safely from the decoded file
                local REMOTE_VERSION=$(grep -m 1 '^VERSION=' "$TEMP_DEC" | tr -d '\r' | cut -d'"' -f2)
                [[ -z "$REMOTE_VERSION" ]] && REMOTE_VERSION="UNKNOWN"
                
                # Clean up version check temp files
                rm -f "$TEMP_RAW" "$TEMP_DEC"

                if [[ "$REMOTE_VERSION" == "UNKNOWN" ]]; then
                    echo -e "  ${DG}────────────────────────────────────────────────────────────${NC}"
                    echo -e "\n  ${R}✘ CRITICAL ERROR: COULD NOT DETECT VERSION IN GITHUB SCRIPT.${NC}"
                    echo -e "  ${Y}⚠ REASON: SCRIPT IS CORRUPTED OR MISSING 'VERSION=' TAG.${NC}"
                    echo -e "  ${DG}UPDATE ABORTED TO PREVENT SYSTEM CRASH (FORK BOMB).${NC}"
                    sleep 4
                    continue
                fi

                echo -e "  ${DG}────────────────────────────────────────────────────────────${NC}"
                echo -e "\n  ${C}▶ CURRENT VERSION : ${W}${VERSION}${NC}"
                
                if [[ "$VERSION" == "$REMOTE_VERSION" ]]; then
                    echo -e "  ${C}▶ LATEST VERSION  : ${G}${REMOTE_VERSION} ${DG}(UP TO DATE)${NC}\n"
                    echo -e "  ${Y}ℹ YOU ALREADY HAVE THE LATEST VERSION INSTALLED.${NC}\n"
                    confirm "FORCE UPDATE ANYWAY? (DATA WILL BE SAFE)" || continue
                else
                    echo -e "  ${C}▶ LATEST VERSION  : ${Y}${REMOTE_VERSION} ${R}(UPDATE AVAILABLE)${NC}\n"
                ...
                (wget -q -O "$TEMP_SCRIPT.raw" "$REPO_URL") & spin $! "DOWNLOADING LATEST SCRIPT"
                sleep 0.3
                
                if [[ -s "$TEMP_SCRIPT.raw" ]]; then
                    (
                        if base64 -d "$TEMP_SCRIPT.raw" > "$TEMP_SCRIPT" 2>/dev/null; then
                            if ! grep -q "TDZ NETWORK CONTROL" "$TEMP_SCRIPT"; then
                                mv "$TEMP_SCRIPT.raw" "$TEMP_SCRIPT"
                            else
                                rm -f "$TEMP_SCRIPT.raw"
                            fi
                        else
                            mv "$TEMP_SCRIPT.raw" "$TEMP_SCRIPT"
                        fi
                        sed -i 's/\r$//' "$TEMP_SCRIPT" 2>/dev/null
                    ) & spin $! "DECODING & OPTIMIZING SCRIPT"
                    
                    if ! grep -q "TDZ NETWORK CONTROL" "$TEMP_SCRIPT"; then
                        echo -e "\n  ${R}✘ ERROR: DOWNLOADED SCRIPT IS CORRUPTED.${NC}"
                        rm -f "$TEMP_SCRIPT" "$TEMP_SCRIPT.raw"
                        sleep 3
                        continue
                    fi
                ...
                echo -e "\n  ${DG}PLEASE RESTART THE DASHBOARD BY TYPING: ${C}clp${NC}"
                echo -e "\n  ${DG}PRESS ENTER TO EXIT...${NC}"
                read -r
                exit 0
                ;;
            2)
...
# ===== TRAFFIC MONITOR (without Total Traffic) =====
live_monitor() {
    while true; do
...
# ===== ENHANCED: Multi-Interface Selection with 0 for Back =====
change_interface() {
    while true; do
        clear; hline; center "${R}⚠  CHANGE NETWORK INTERFACE${NC}"; hline; echo
        
        # Get all interfaces (excluding lo, but including tun, wg, etc.)
        local ifaces=()
        ...
        # Determine next number for custom
        ...
        # If Enter pressed, just loop again (no message)
        if [[ -z "$if_choice" ]]; then
            continue
        fi
